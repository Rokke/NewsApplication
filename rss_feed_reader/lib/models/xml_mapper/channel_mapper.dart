...
class ChannelMapper extends XMLBaseMapper {
  ...
  /// Parse RSS XML from channel info. This is the feed info and not the articles
  factory ChannelMapper.fromXML(XmlElement xEl) {
    _log.finest('ChannelMapper.fromXML');
    ...
    for (final xNode in xEl.children) {
      try {
        if (xNode is XmlElement) {
          switch (xNode.name.toString()) {
            ...
            case 'itunes:category':
              final text = (xNode.innerText.isEmpty ? xNode.getAttribute('text') ?? '' : xNode.innerText).trim();
              if (text.isNotEmpty && !text.contains('|')) {
                channel.category = (channel.category?.isEmpty ?? true) ? text : '${channel.category},$text';
              }
              break;
            case 'generator':
        ...
        } else if (xNode.nodeType != XmlNodeType.TEXT || xNode.value!.trim().isNotEmpty) {
          _log.info('Ukjent nodetype: ${xNode.nodeType}=>${xNode.outerXml}');
        }
      } catch (err, stack) {
        _log.warning('ChannelMapper.fromXML-$xNode', err, stack);
        rethrow;
      }
    }
    return channel;
