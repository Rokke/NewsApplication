          ...
          child: GestureDetector(
            onTap: () async {
              ...
              if (ret is int) {
                ...
                if (category != null) {
                  await reader(rssDatabase).updateCategory(categoryId: category.id, categoryCompanion: CategoryCompanion(name: moor.Value(categoryName), displayName: moor.Value(category.displayName), color: moor.Value(ret)));
                } else {
                  await reader(rssDatabase).insertCategory(CategoryCompanion.insert(name: categoryName, displayName: moor.Value(categoryName), color: moor.Value(ret)));
                }
              }
            },
            child: Hero(
